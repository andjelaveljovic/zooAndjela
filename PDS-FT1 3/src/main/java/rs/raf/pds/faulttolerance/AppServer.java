...
public class AppServer extends SyncPrimitive implements Runnable, ReplicatedLog.LogReplicator{
	...
	public String findNodeNameByChannel(FollowerGRPCChannel targetChannel) {
		for (Map.Entry<String, FollowerGRPCChannel> entry : followersChannelMap.entrySet()) {
			String nodeName = entry.getKey();
			FollowerGRPCChannel grpcChannel = entry.getValue();

			if (grpcChannel == targetChannel) {
				return nodeName; // Found the node name corresponding to targetChannel
			}
		}
		return null; // Return null if targetChannel not found
	}
		...
		for (int i=1; i<nodeList.size();i++) {
			...
			try {
				if (followerChannel == null) {
					...
					System.out.println("Current followers:");
					for (String nodeNameProba : followersChannelMap.keySet()) {
	...
	public void replicateOnFollowers(Long entryAtIndex, byte[] data) {
		...
		for (FollowerGRPCChannel grpcChannel:followersChannelMap.values()) {
			LogResponse	 response = grpcChannel.blockingStub.appendLog(logEntry);
			String nodeName = findNodeNameByChannel(grpcChannel);
			System.out.println("Saljem na **************" +nodeName);

			switch (response.getStatus()) {
				case LOG_OK:
					System.out.println("Log operation successful");
					break;
				case IO_ERROR:
					System.out.println("IO Error occurred");
					break;
				case LOG_HASNT_LAST_ENTRY:
					System.out.println("Log hasn't last entry; lastEntryIndex = " + response.getEntryAtIndex());
					break;
				default:
